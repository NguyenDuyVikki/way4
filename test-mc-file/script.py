...
def encrypt_file(xml_files: list, key: str, dir_path, log_file_path, gpg_dir_path) -> bool:
    try:
        print(f"📁 GPG directory path: {gpg_dir_path}")
        # Full path to gpg.exe
        gpg_binary_path = os.path.join(gpg_dir_path, 'bin', 'gpg.exe')
        
        print(f"🔍 Using gpg binary: {gpg_binary_path}")

        # Initialize GPG with custom binary path
        gpg = gnupg.GPG(gnupghome=gpg_dir_path, gpgbinary=gpg_binary_path)
...
def load_config(config_path=None):
    ...
    try:
        ...
        required_keys = ['input_folder', 'output_folder', 'file_types', 'gpg_key', 'checkpoint_file', 'dir_log', 'gpg_dir']
        ...
        config['dir_log'] = str(Path(config['dir_log']).resolve())
        config['gpg_dir'] = str(Path(config['gpg_dir']).resolve())
        return config
    except Exception as e:
...
def main():
    ...
    try:
        ...
        for file, _, _ in new_files:
            ...
            if mc_file:  
                status = encrypt_file([mc_file], gpg_key, dir_encrypt_path, log_file_path, gpg_dir_path)
                if status:
